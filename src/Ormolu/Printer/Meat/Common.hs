...
p_hsDoc' poHStyle hstyle needsNewline (L l str) = do
  -- let isHaddockSpan = \case
  --       HaddockSpan _ -> True
  --       _ -> False
  -- goesAfterHaddock <- maybe False isHaddockSpan <$> getSpanMark
  -- -- Make sure the Haddock is separated by a newline from Haddock comments.
  -- when goesAfterHaddock newline
